    ...
    def wrapped(bot, update, *args, **kwargs):
        ...
        if user_id != chat_id:
            bot.send_message(chat_id=update.message.chat_id,
                             text="In order to post this command you must address the bot directly.\nTry resending in %s" % (
                                 bot.name))
            return
...
def intro(bot, update):
    ...
    bot.send_message(chat_id=update.message.chat_id,
                     text="Hello everybody. Welcome to `THE CLUE`. Please everybody say /hi")
...
def guess_final(game, user_id, bot, update, user_data):
    ...
    if user_data["type"] == "Suggest":
        who_will_show, what = game.suggest(user_id, user_data["suspect"],
                                           user_data["tool"], user_data["room"])

        bot.send_message(chat_id=game._chat_id,
                         text="%s has suggested: %s, %s, %s " % (suggestor.name, user_data["suspect"],
                                                                 user_data["tool"], user_data["room"]))
    ...
    if user_data["type"] == "Accuse":
        res = game.accuse(user_data["suspect"],
                          user_data["tool"],
                          user_data["room"])

        bot.send_message(chat_id=game._chat_id,
                         text="%s has accused : %s, %s, %s " % (suggestor.name,
                                                                user_data["suspect"],
                                                                user_data["tool"],
                                                                user_data["room"]))
...
def show_card(game, user_id, bot, update, user_data):
    user = game.get_user(user_id)
    card_to_show = update.message.text.split(":", 1)[1]
    bot.send_message(chat_id=user_data["show_to"],
                     text="%s has shown you %s" % (user.name, card_to_show))

    if card_to_show in game._deck:
        card_obj = game._deck[game._deck.index(card_to_show)]
        send_card(bot, user_data["show_to"], card_obj)
...
def set_dispatchr(server_updater):
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ChooseState.PLAYER: [MessageHandler(Filters.text, guess_suspect, pass_user_data=True)],
